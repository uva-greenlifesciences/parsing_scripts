...
# import file with viridiplantae species + complete name correspondence
#with open("viridiplantae.wo_sly.txt","r") as filin:
#	lines = filin.readlines()
#	lines_parsed = [line.strip() for line in lines]
#species = [line[0:4].strip() for line in lines_parsed]

# filtering by viridiplantae species
#with open("mature.fa","r") as filin, open("mature.viridiplantae.fa","w") as fileout:
#	for record in SeqIO.parse(filin,"fasta"):
#		for specie in species:
#			if specie in record.id[0:3]:
#				SeqIO.write(record,fileout,"fasta")

#with open("hairpin.fa","r") as filin, open("hairpin.viridiplantae.fa","w") as fileout:
#	for record in SeqIO.parse(filin,"fasta"):
#		for specie in species:
#			if specie in record.id[0:3]:
#				SeqIO.write(record,fileout,"fasta")
...
# converting each record Seq alphabet to IUPACUnambiguousRNA
#with open("hairpin.viridiplantae.fa","r") as filin, open("hairpin.viridiplantae.iupacrna.fa","w") as fileout:
#	rnas = []	
#	for record in SeqIO.parse(filin,"fasta"):
#		record.seq = Seq(str(record.seq),IUPAC.unambiguous_rna)
#		rnas.append(record)
#	SeqIO.write(rnas,fileout,"fasta")
		
#with open("mature.viridiplantae.fa","r") as filin, open("mature.viridiplantae.iupacrna.fa","w") as fileout:
#	rnas = []	
#	for record in SeqIO.parse(filin,"fasta"):
#		record.seq = Seq(str(record.seq),IUPAC.unambiguous_rna)
#		rnas.append(record)
...
# converting each record sequence to DNA (IUPACUnambiguousDNA)
#with open("hairpin.viridiplantae.iupacrna.fa","r") as filin, open("hairpin.viridiplantae.dna.fa","w") as fileout:
#	dnas = []	
#	for record in SeqIO.parse(filin,"fasta"):
#		dna = record.seq.back_transcribe() # reverse transcribe RNA --> DNA
#		record.seq = dna
#		dnas.append(record)
#	SeqIO.write(dnas,fileout,"fasta")

#with open("mature.viridiplantae.iupacrna.fa","r") as filin, open("mature.viridiplantae.dna.fa","w") as fileout:
#	dnas = []	
#	for record in SeqIO.parse(filin,"fasta"):
#		dna = record.seq.back_transcribe() # reverse transcribe RNA --> DNA
#		record.seq = dna
...
# Converting to one fasta header + one sequence per line 
#call("fasta_formatter -i mature.viridiplantae.dna.fa -o mature.viridiplantae.dna.oneline.fa",shell=True)
#call("fasta_formatter -i hairpin.viridiplantae.dna.fa -o hairpin.viridiplantae.dna.oneline.fa",shell=True)

# Removing redundancy among redundant sequences in mature + hairpin fasta files
#sequence_cleaner("mature.viridiplantae.dna.fa")
#sequence_cleaner("hairpin.viridiplantae.dna.fa")



# Create bowtie-2 indexes
#call("bowtie2-build collapsed.hairpin.viridiplantae.dna.fa collapsed.hairpin.viridiplantae.dna.fa",shell=True)
#call("bowtie2-build collapsed.mature.viridiplantae.dna.fa collapsed.mature.viridiplantae.dna.fa",shell=True)
